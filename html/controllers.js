...
app.controller("MainController", function(Websocket, Tables, SceneDrawing, ChartDrawing){
    ...
    Websocket.Ws.$on('spike', function (data) {
        ChartDrawing.spike(data.id, data.term);
    });
});

app.controller("WidgetController", function(Websocket, Tables, ChartDrawing){
    ...
    this.addToWatch = function(){
        ...
        //add chart to chart widget
        ChartDrawing.add(data.id);

        //send subscribe request to WS server
